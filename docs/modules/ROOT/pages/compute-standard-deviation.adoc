...
:description: In this tutorial, you'll build an application that calculates the standard deviation of customer satisfaction scores, using code that's executed on each member of a Hazelcast Viridian cluster.
...
In this tutorial, you'll deploy the following to a {hazelcast-cloud} Serverless cluster:

- A `Customer` class to store `Customer` objects in a map. The objects will include a customer satisfaction score.
- An entry processor that the cluster will use to calculate the standard deviation of all customer satisfaction scores in the map.
...
The client code shows both the junior developer and senior developer approach. The junior developer approach does the processing on the client-side. The senior developer approach is more efficient because it offloads the processing to the {hazelcast-cloud} Serverless cluster, using the entry processor.
...
The important parts to understand are the differences between the junior developer code and the senior developer code.

In each example, the cluster stores five `Customer` objects, and the simplest Hazelcast cluster has one member.

The junior developer code must request five integers from the cluster in order to run the calculation. The senior developer code runs the calculation on the cluster and returns one double across the network. So the choice is five network transfers or one.

Now imagine that you have 1,000,000 `Customer` objects stored on two Hazelcast members. One member could handle this volume but wouldn't be resilient to failure. So the junior developer approach now requests 1,000,000 integers from the cluster across the network and the senior developer approach retrieves two doubles, one from each member.

It's clear that if data volumes increase, the cluster-side computation copes better. The same amount of data still has to be examined, but less data moves.
...
link:{github-directory}/client/src/main/java/your/company/name/JuniorDeveloper.java[View source]
====
...
link:{github-directory}/client/src/main/java/your/company/name/SeniorDeveloper.java[View source]
====
...
. Open the `pom.xml` file in the `hazelcast-cloud-code-samples/compute/standard-deviation/cluster-side` directory.

. Configure the Maven plugin with values for the following elements:
+
[cols="1a,1a"]
|===
|Element|Location in Hazelcast {hazelcast-cloud} console

|`<clusterName>`
|Go to *Connect Your Application* > *Advanced setup*. The cluster name is at the top of the list.
...
. Change into the `hazelcast-cloud-code-samples/compute/standard-deviation/cluster-side` directory.
...
. Open the link:{page-cloud-console}[Hazelcast {hazelcast-cloud} console].

. Go to *Connect Your Application* > *Advanced setup*, and click *Download keystore file*.

. Extract the files and copy the `client.keystore` and `client.truststore` files to the `client/src/main/resources` directory.

. Leave the Hazelcast {hazelcast-cloud} console open. You'll need some of these details in next steps.

. Change into the `hazelcast-cloud-code-samples/compute/standard-deviation/client` directory.
...
. When prompted, enter the cluster name, keystore password, and discovery token for your cluster. These details are in the *Adavnced setup* tab in the Hazelcast {hazelcast-cloud} console.
...
- Use an entry processor to process data in parallel on a {hazelcast-cloud} Serverless cluster.
- Deploy cluster-side modules to a {hazelcast-cloud} Serverless cluster using the Hazelcast {hazelcast-cloud} Maven plugin.
